...
-- Spark pass-through query). Delivery is at-least-once: duplicates are
-- possible after a Spark restart, so there is intentionally no unique
-- constraint on (venue, matched_event_id, timestamp).
...
CREATE TABLE IF NOT EXISTS mispricing_signals (
    ...
    "timestamp"            TIMESTAMPTZ NOT NULL, -- event time of the paired readings
...
CREATE TABLE IF NOT EXISTS daily_signal_summary (
    ...
    reading_pairs    INTEGER NOT NULL DEFAULT 0, -- price readings observed that day (both venues)
